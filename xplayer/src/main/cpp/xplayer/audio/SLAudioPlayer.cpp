...
SLAudioPlayer::SLAudioPlayer(const std::shared_ptr<PlayerState> &playerState) : IAudioPlay(
        playerState) {
    buffer = new unsigned char[1024 * 1024];
}

static void pcmCallback(SLAndroidSimpleBufferQueueItf bf, void *context) {
    auto *slAudioPlayer = (SLAudioPlayer *) context;
    if (!slAudioPlayer) {
...
bool SLAudioPlayer::startPlay(DecodeParam out) {
    engineItf = createSL();
    if (!engineItf) {
        LOGE("createSL ERROR");
        return false;
...
void SLAudioPlayer::playCall(void *bufferQueue) {
...
SLuint32 SLAudioPlayer::getCurSampleRate(int sample_rate) {
    SLuint32 rate = 0;
    switch (sample_rate) {
        case 8000:
